...
const getSensorStatus = (value: number, type: string): "normal" | "warning" | "alarm" => {
  if (type === "temperature") {
...
export default function MonitorView() {
  ...
  const calculateStats = useCallback((data: number[], type: string, current: number): SensorStats => {
    if (data.length === 0) return { current, min: current, max: current, avg: current, status: "normal" };
    const min = Math.min(...data);
    const max = Math.max(...data);
    const avg = data.reduce((a, b) => a + b, 0) / data.length;
    return { current, min, max, avg, status: getSensorStatus(current, type) };
  }, []);
  ...
  const updateChart = useCallback((data: ChartData) => {
    ...
    setLatestData(data);
    setStats({
      temperature: calculateStats(temps, "temperature", data.temperature),
  ...
  useEffect(() => {
    ...
    if (!isTauri()) {
      console.warn("Not running in Tauri environment - sensor data will not be available");
      return;
    }
  ...
  return (
    <div className={styles.view}>
      ...
      <div className={monitorStyles.statsGrid}>
        <div className={monitorStyles.statCard} data-type="temperature" data-status={stats.temperature.status}>
          <div className={monitorStyles.cardHeader}>
            ...
            <span className={monitorStyles.statusBadge} data-status={stats.temperature.status}>
        ...
        <div className={monitorStyles.statCard} data-type="pressure" data-status={stats.pressure.status}>
          <div className={monitorStyles.cardHeader}>
            <div className={monitorStyles.cardIcon}>
              <svg viewBox="0 0 24 24" fill="currentColor">
            ...
            <span className={monitorStyles.statusBadge} data-status={stats.pressure.status}>
        ...
        <div className={monitorStyles.statCard} data-type="humidity" data-status={stats.humidity.status}>
          <div className={monitorStyles.cardHeader}>
            <div className={monitorStyles.cardIcon}>
              <svg viewBox="0 0 24 24" fill="currentColor">
            ...
            <span className={monitorStyles.statusBadge} data-status={stats.humidity.status}>
      ...
      <div className={monitorStyles.chartContainer}>
        <div className={monitorStyles.chartHeader}>
          ...
          <div className={monitorStyles.chartControls}>
            <div className={monitorStyles.timeRangeGroup}>
              {[60, 120, 300].map((range) => (
                <button
                  ...
                  onClick={() => setTimeRange(range as 60 | 120 | 300)}
                >
                  {range === 60 ? "1m" : range === 120 ? "2m" : "5m"}
                </button>
              ))}
            </div>
            <button
            ...
            <button className={monitorStyles.controlBtn} onClick={handleClearData}>
              <svg viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className={monitorStyles.chartLegend}>
          <div className={monitorStyles.legendItem} data-type="temperature">
            <span className={monitorStyles.legendDot} />
            Temperature
          </div>
          <div className={monitorStyles.legendItem} data-type="pressure">
            <span className={monitorStyles.legendDot} />
            Pressure
          </div>
          <div className={monitorStyles.legendItem} data-type="humidity">
            <span className={monitorStyles.legendDot} />
            Humidity
          </div>
        </div>
