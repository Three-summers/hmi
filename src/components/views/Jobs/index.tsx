...
const StatusIcons: Record<Job["status"], JSX.Element> = {
  idle: <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8z"/></svg>,
  running: <svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>,
  ...
  paused: <svg viewBox="0 0 24 24" fill="currentColor"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>,
};
...
export default function JobsView() {
  ...
  const getStatusColor = (status: Job["status"]) => {
    switch (status) {
      ...
      case "completed": return "attention";
    ...
    <div className={sharedStyles.view}>
      <div className={styles.statsBar}>
        ...
        <div className={styles.statItem} data-status="processing">
          ...
          <span className={styles.statLabel}>{t("jobs.running")}</span>
        </div>
        <div className={styles.statItem} data-status="attention">
          <span className={styles.statValue}>{completedJobs}</span>
          <span className={styles.statLabel}>{t("jobs.completed")}</span>
        </div>
          ...
          {jobs.map((job) => (
            ...
            >
              <div className={styles.jobHeader}>
                <div className={styles.jobStatus} data-status={getStatusColor(job.status)}>
                  {StatusIcons[job.status]}
                </div>
                <div className={styles.jobTitle}>
                  <span className={styles.jobName}>{job.name}</span>
                  <span className={styles.jobRecipe}>{job.recipe}</span>
                </div>
              ...
              <div className={styles.jobDetails}>
                <div className={styles.detailItem}>
                  <span className={styles.detailLabel}>Wafers</span>
                  <span className={styles.detailValue}>{job.completedWafers}/{job.waferCount}</span>
                </div>
                <div className={styles.detailItem}>
                  <span className={styles.detailLabel}>Start</span>
                  <span className={styles.detailValue}>{formatTime(job.startTime)}</span>
                </div>
                <div className={styles.detailItem}>
                  <span className={styles.detailLabel}>ETA</span>
                  <span className={styles.detailValue}>{formatTime(job.estimatedEnd)}</span>
                </div>
              </div>
            </div>
          ))}
